...
class ScanRegistrationNode : public rclcpp::Node {
 public:
  ScanRegistrationNode() : Node("scan_registration") {
    ...
    pubFullLaserCloud = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "/livox_full_cloud", 10);
    pubSharpCloud = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "/livox_less_sharp_cloud", 10);
    pubFlatCloud = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "/livox_less_flat_cloud", 10);
    pubNonFeature = this->create_publisher<sensor_msgs::msg::PointCloud2>(
    ...
    if (Lidar_Type == 0 || Lidar_Type == 1) {
      ...
      subPointCloud = this->create_subscription<sensor_msgs::msg::PointCloud2>(
          "/livox/lidar", 100,
          std::bind(&ScanRegistrationNode::pointCloudCallback, this,
                    std::placeholders::_1));
 ...
 private:
  ...
  void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    lidarFeatureExtractor->FeatureExtract_Mid(laser_cloud_custom,
                                              laserConerCloud, laserSurfCloud);
