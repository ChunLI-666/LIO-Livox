 ...
 */
IMUIntegrator::IMUIntegrator(std::vector<sensor_msgs::msg::Imu::SharedPtr> vIMU)
...
const Eigen::Quaterniond& IMUIntegrator::GetDeltaQ() const {
  return dq;
}

const Eigen::Vector3d& IMUIntegrator::GetDeltaP() const {
  return dp;
}

const Eigen::Vector3d& IMUIntegrator::GetDeltaV() const {
  return dv;
}

const double& IMUIntegrator::GetDeltaTime() const {
  return dtime;
}
...
void IMUIntegrator::GyroIntegration(double lastTime) {
  ...
  for (auto& imu : vimuMsg) {
    ...
    double dt = (imu->header.stamp.sec + imu->header.stamp.nanosec * 1e-9) -
                current_time;
    assert(dt >= 0);
