...
void signalHandler(int signal) {
  if (signal == SIGINT || signal == SIGTERM) {
    LIO_LOG_INFO << "Received signal " << signal
                 << ". Saving map before exit...";

    if (g_estimator != nullptr) {
      std::string output_dir =
          "/home/charles/project/LIO-Livox/mapping_results";
...
class PoseEstimationNode : public rclcpp::Node {
 public:
  PoseEstimationNode() : Node("pose_estimation") {
    ...
    if (vecTlb.size() < 12) {
      LIO_LOG_ERROR
          << "Extrinsic_Tlb parameter must have at least 12 elements, got "
          << vecTlb.size();
    ...
    pubLaserOdometry = this->create_publisher<nav_msgs::msg::Odometry>(
        "/livox_odometry_mapped", 5);
    pubLaserOdometryPath = this->create_publisher<nav_msgs::msg::Path>(
        "/livox_odometry_path_mapped", 5);
    pubFullLaserCloud = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "/livox_full_cloud_mapped", 10);
    pubGps =
        this->create_publisher<sensor_msgs::msg::NavSatFix>("/lidar", 1000);

    // Create subscribers
    subFullCloud = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        "/livox_full_cloud", 10,
        std::bind(&PoseEstimationNode::fullCallBack, this,
                  std::placeholders::_1));

    if (IMU_Mode > 0) {
      sub_imu = this->create_subscription<sensor_msgs::msg::Imu>(
          "/livox/imu", 2000,
          std::bind(&PoseEstimationNode::imu_callback, this,
                    std::placeholders::_1));
    }
 ...
 private:
  ...
  void pubOdometry(const Eigen::Matrix4d& newPose, double& timefullCloud) {
    ...
    laserOdometry.header.stamp =
        rclcpp::Time(static_cast<int64_t>(timefullCloud * 1e9));
    ...
    laserOdometryTrans.header.stamp =
        rclcpp::Time(static_cast<int64_t>(timefullCloud * 1e9));
  ...
  bool fetchImuMsgs(double startTime, double endTime,
    ...
    while (true) {
      ...
      if (_imuMsgQueue.back()->header.stamp.sec +
                  _imuMsgQueue.back()->header.stamp.nanosec * 1e-9 <
              endTime ||
          _imuMsgQueue.front()->header.stamp.sec +
                  _imuMsgQueue.front()->header.stamp.nanosec * 1e-9 >=
              endTime)
        break;
      sensor_msgs::msg::Imu::SharedPtr& tmpimumsg = _imuMsgQueue.front();
      double time =
          tmpimumsg->header.stamp.sec + tmpimumsg->header.stamp.nanosec * 1e-9;
      if (time <= endTime && time > startTime) {
        ...
        if (time <= startTime) {
          ...
          theLastIMU->header.stamp =
              rclcpp::Time(static_cast<int64_t>(endTime * 1e9));
    ...
    for (int i = 1; i < v_size; i++) {
      ...
      problem.AddResidualBlock(Cost_Initialization_IMU::Create(
                                   ...
                                   Eigen::LLT<Eigen::Matrix<double, 9, 9>>(
                                       ...
                                       .matrixL()
                                       .transpose()),
                               nullptr, para_r, para_v[i - 1], para_v[i],
                               para_ba, para_bg);
    }
  ...
  void process() {
    ...
    while (rclcpp::ok()) {
      ...
      if (!_lidarMsgQueue.empty()) {
        // get new lidar msg
        time_curr_lidar = _lidarMsgQueue.front()->header.stamp.sec +
                          _lidarMsgQueue.front()->header.stamp.nanosec * 1e-9;
      ...
      if (newfullCloud) {
        LIO_LOG_INFO << "[PoseEstimation] Processing new lidar frame, time: "
                     << time_curr_lidar
                     << ", LidarIMUInited: " << LidarIMUInited;
        ...
        if (IMU_Mode > 0 && time_last_lidar > 0) {
          ...
          while (!fetchImuMsgs(time_last_lidar, time_curr_lidar, vimuMsg)) {
            countFail++;
            if (countFail > 100) {
              LIO_LOG_WARNING << "[PoseEstimation] Failed to fetch IMU data "
                                 "after 100 retries, time range: ["
          ...
          if (!vimuMsg.empty()) {
            LIO_LOG_DEBUG << "[PoseEstimation] Fetched " << vimuMsg.size()
                          << " IMU messages";
          }
        }
        ...
        if (!vimuMsg.empty()) {
          if (!LidarIMUInited) {
            ...
            lidarFrame.P = Pwbpre + Vwbpre * dt +
                           0.5 * GravityVector * dt * dt + Qwbpre * (dP);
          ...
          if (LidarIMUInited) {
            ...
            LIO_LOG_WARNING << "[PoseEstimation] No IMU data but "
                               "LidarIMUInited=true, skipping this frame";
            time_last_lidar = time_curr_lidar;
            continue;
          } else {
            // predict current lidar pose
        ...
        LIO_LOG_INFO
            << "[PoseEstimation] Calling EstimateLidarPose, lidar_list size: "
            << lidar_list->size();
        ...
        Eigen::Vector3d Pwlpre =
            transformAftMapped.topLeftCorner(3, 3) * exPbl +
            transformAftMapped.topRightCorner(3, 1);
        ...
        int laserCloudFullResNum =
            lidar_list->front().laserCloud->points.size();
        LIO_LOG_INFO << "[PoseEstimation] Publishing point cloud with "
                     << laserCloudFullResNum << " points";
        ...
        laserCloudMsg.header.stamp = rclcpp::Time(
            static_cast<int64_t>(lidar_list->front().timeStamp * 1e9));
        pubFullLaserCloud->publish(laserCloudMsg);
        LIO_LOG_INFO << "[PoseEstimation] Point cloud published to "
                        "/livox_full_cloud_mapped";
        ...
        if (IMU_Mode > 1 && !LidarIMUInited) {
          ...
          if (pushCount >= 3) {
            ...
            if (lidarFrameList->size() == int(WINDOWSIZE / 1.5)) {
              ...
              LIO_LOG_INFO << "[PoseEstimation] Set startTime to: " << startTime
                           << ", WINDOWSIZE: " << WINDOWSIZE;
            }

            if (!LidarIMUInited && lidarFrameList->size() == WINDOWSIZE &&
                lidarFrameList->front().timeStamp >= startTime) {
              LIO_LOG_INFO << "**************Start MAP "
                              "Initialization!!!******************";
              LIO_LOG_INFO << "[PoseEstimation] Starting MAP Initialization, "
                              "lidarFrameList size: "
                           << lidarFrameList->size();
              if (TryMAPInitialization()) {
                ...
                LIO_LOG_WARNING
                    << "[PoseEstimation] MAP Initialization FAILED!";
              }
              LIO_LOG_INFO << "**************Finish MAP "
                              "Initialization!!!******************";
            }
          }
        }
...
int main(int argc, char** argv) {
  ...
  lio_livox::Logger::Initialize("LioLivox",
                                "/home/charles/project/LIO-Livox/logs", "INFO");
  ...
  LIO_LOG_INFO
      << "PoseEstimation node started. Press Ctrl+C to save map and exit.";
