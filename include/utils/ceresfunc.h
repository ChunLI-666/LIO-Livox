...
#define INCLUDE_UTILS_CERESFUNC_H_
...
struct ThreadsStruct {
  ...
  Eigen::MatrixXd A;
  Eigen::VectorXd b;
  std::unordered_map<intptr_t, int> parameter_block_size;
  std::unordered_map<intptr_t, int> parameter_block_idx;
};
 ...
 public:
  ...
  void addResidualBlockInfo(ResidualBlockInfo* residual_block_info) {
    ...
    for (int i = 0;
      ...
      parameter_block_size[reinterpret_cast<intptr_t>(addr)] = size;
    }

    for (int i = 0; i < static_cast<int>(residual_block_info->drop_set.size());
         i++) {
      double* addr = parameter_blocks[residual_block_info->drop_set[i]];
      parameter_block_idx[reinterpret_cast<intptr_t>(addr)] = 0;
    }
  }
  ...
  void preMarginalize() {
    for (auto it : factors) {
      ...
      for (int i = 0; i < static_cast<int>(block_sizes.size()); i++) {
        intptr_t addr = reinterpret_cast<intptr_t>(it->parameter_blocks[i]);
  ...
  void marginalize() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
      ...
      int ret = pthread_create(&tids[i], NULL, ThreadsConstructA,
                               reinterpret_cast<void*>(&(threadsstruct[i])));
      if (ret != 0) {
  ...
  int sum_block_size;
  std::unordered_map<intptr_t, int> parameter_block_idx;
  std::unordered_map<intptr_t, double*> parameter_block_data;
...
struct Cost_Initial_G {
  ...
  explicit Cost_Initial_G(Eigen::Vector3d acc_) : acc(acc_) {}
